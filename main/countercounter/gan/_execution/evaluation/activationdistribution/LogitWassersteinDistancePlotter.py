...
class LogitWassersteinDistancePlotter(WassersteinDistancePlotter):

    def __init__(self, wasserstein_distances, path):
        super().__init__(wasserstein_distances, path)

    def _print_distances(self):
        lines = [
            f'Avg Wasserstein distance over all classes and logits: {self.mean_wasserstein_distance:.4f}\n',
            '\n'
        ] + [
            f'Avg Wasserstein distance over all classes and logits: {self.mean_wasserstein_distance:.4f}\n',
            '\n'
            ...
            f'Wasserstein distance for logit {feature}: {self.wasserstein_distance_by_feature[feature]:.4f}\n' for feature in sorted(self.wasserstein_distance_by_feature)
        ] + [
            '\n'
        ] + [
            f'Wasserstein distance for logit {feature} and class {label}: {self.wasserstein_distance_by_feature_by_class[feature][label]:.4f}\n'
        ...
        with open(f'{self.output_folder}{sep}wasserstein_mean_logit_distances.txt', 'w') as file:
    ...
    def _plot_mean_distance_by_class(self):
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_class_logits')
        plt.close()
    ...
    def plot_distance_by_feature(self):
        ...
        bar = ax.bar(x, y)
        ax.set_title('Wasserstein Distance Original - Generated Distributions by Logit')
        ax.bar_label(bar, padding=3, fmt='%.2f')
        ax.set_xticks(x)
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logit', bbox_inches="tight")
        plt.close()
    ...
    def plot_distance_by_feature_by_class(self):
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logit_and_class.png')
        plt.close()
    ...
    def plot_distances_between_classes_within_datasets(self, wasserstein_distances_between_classes_within_datasets):
        ...
        with open(f'{self.output_folder}{sep}wasserstein_distances_between_classes_within_dataset_logits.txt', 'w') as file:
            file.writelines(lines)
    ...
    def _barplot_distances(self, distances, dataset_name):
        ...
        bar = ax.bar(x, y)
        ax.set_title(f'Wasserstein Distance between Class 0 and Class 1 Logit Distributions -- {dataset_name}')
        ax.bar_label(bar, padding=3, fmt='%.2f')
        ax.set_xticks(x)
        ax.set_xticklabels([f'{idx}' for idx in x])
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logit_within_dataset_{dataset_name}')
        plt.close()
    ...
    def _joint_barplot_distances(self, test_distances, gen_distances):
        ...
        ax.set_title('Wasserstein Distance between Class 0 and Class 1 Logit Distributions')

        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logit_within_dataset_comparison.png')
        plt.close()
    ...
    def _barplot_distance_differences(self, test_distances, gen_distances):
        ...
        bar = ax.bar(x, y)
        ax.set_title(f'Differences in Wasserstein Distance between Class 0 and Class 1 Logit Distributions Test and Generated')
        ax.bar_label(bar, padding=3, fmt='%.2f')
        ax.set_xticks(x)
        ax.set_xticklabels([f'{idx}' for idx in x])
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_difference_within_dataset_logits')
        plt.close()
    ...
    def plot_distances_between_classes_between_datasets(self, wasserstein_distances_between_datasets):
        ...
        lines = [
            'Comparing the Distance between the logit distribution for two classes between test and generated set.\n',
        ...
        with open(f'{self.output_folder}{sep}wasserstein_distances_between_classes_between_dataset_logits.txt', 'w') as file:
            file.writelines(lines)
    ...
    def _barplot_distances_between_datasets(self, distances, name):
        ...
        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logits_between_dataset_{name}')
        plt.close()
    ...
    def _joint_barplot_distances_between_datasets(self, test0_test1_dist, gen0_test1_dist, gen1_test0_dist):
        ...
        ax.set_title('Wasserstein Distance between Logit Distributions')

        plt.savefig(f'{self.output_folder}{sep}Wasserstein_distance_by_logit_between_dataset_comparison.png')
        plt.close()
