...
class GPix2PixSmallNoUnet(nn.Module):

    def __init__(self, in_channels=1, out_channels=1, input_size=128, use_conv2dtranspose=True, upsample_mode='nearest', more_noise=False):
        super().__init__()
        ...
        self.e7 = nn.Conv2d(512, 512, kernel_size=self.kernel, stride=self.stride, padding=self.padding_conv2d)
        self.e7_act = nn.ReLU()

        self.d1, self.d1_act = self.decoder_block(512, 512, dropout=True)
        self.d2, self.d2_act = self.decoder_block(512, 512, dropout=True)
        self.d3, self.d3_act = self.decoder_block(512, 512, dropout=True)
        self.d4, self.d4_act = self.decoder_block(512, 256, dropout=self.more_noise)
        self.d5, self.d5_act = self.decoder_block(256, 128, dropout=self.more_noise)
        self.d6, self.d6_act = self.decoder_block(128, 64)

        self.d7 = nn.Sequential(
            nn.ConvTranspose2d(64, 1, kernel_size=self.kernel, stride=self.stride, padding=self.padding_conv2d),
            nn.Tanh()
        )
    ...
    def forward(self, x, labels):
        ...
        self.assert_size(e7_out, 512, 1)

        d1_raw = self.d1(e7_out)
        d1_out = self.d1_act(d1_raw)

        d2_raw = self.d2(d1_out)
        d2_out = self.d2_act(d2_raw)

        d3_raw = self.d3(d2_out)
        d3_out = self.d3_act(d3_raw)

        d4_raw = self.d4(d3_out)
        d4_out = self.d4_act(d4_raw)

        d5_raw = self.d5(d4_out)
        d5_out = self.d5_act(d5_raw)

        d6_raw = self.d6(d5_out)
        d6_out = self.d6_act(d6_raw)

        output_out = self.d7(d6_out)
