...
class GPix2PixSmall(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, input_size=128, use_conv2dtranspose=True, upsample_mode='nearest', more_noise=False):
        ...
        self.d4, self.d4_act = self.decoder_block(1024, 256, dropout=self.more_noise)
        self.d5, self.d5_act = self.decoder_block(512, 128, dropout=self.more_noise)
        self.d6, self.d6_act = self.decoder_block(256, 64)

        self.d7 = nn.Sequential(
            nn.ConvTranspose2d(128, self.out_channels, kernel_size=self.kernel, stride=self.stride, padding=self.padding_conv2d),
            nn.Tanh()
        )
